...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  saveLayoutConfig: (filePath: string, layoutConfig: unknown) =>
  ...
  onOpenRecentFile: (callback: (event: unknown, filePath: string) => void) =>
    ipcRenderer.on("open-recent-file", callback),
  onCloseDashboard: (callback: (event: unknown) => void) => ipcRenderer.on("close-dashboard", callback),
});
